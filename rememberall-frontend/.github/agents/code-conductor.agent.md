...
tools:
  [
    ...
    agent/runSubagent,
    mcp-svelte/get-documentation,
    mcp-svelte/list-sections,
    mcp-svelte/playground-link,
...
2. **Delegate Research**: Use #tool:agent to invoke the planning-subagent for comprehensive context gathering. Instruct it to work autonomously without pausing.
...
1. Use #tool:agent to invoke the implement-subagent with:
...
1. Use #tool:agent to invoke the code-review-subagent with:
...
Provide this status in your responses to keep the user informed. Use the #tool:todo tool to track progress.
</state_tracking>
